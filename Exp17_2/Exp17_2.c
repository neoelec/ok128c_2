...
static const uint8_t font[95][5] PROGMEM = {    /* 5x7 ASCII character font */
...
static uint8_t xcharacter, ycharacter;          // x character(0-7), y character(0-19)
static uint8_t cursor_flag, xcursor, ycursor;   // x and y cursor position(0-7, 0-19)

static void GLCD_command(uint8_t signal, uint8_t command)
{                                               /* write a command */
    LCD_CONTROL = signal & 0xC0;                // D/-I = 0
    asm volatile ("push     r0");               // delay for 500ns
    asm volatile ("pop      r0");
...
static void GLCD_xy(uint8_t x, uint8_t y)
...
static void GLCD_character(uint8_t character)
{                                               /* display a character */
...
static uint8_t visit_flag, x, y, table[8][20]; // visiting count variables

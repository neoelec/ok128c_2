...
static void LCD_3dec(uint8_t number)
...
static void Set_timer1(uint8_t number)
{                                               /* set Timer1 OCR1A */
...
int main(void)
{
    ...
    TCCR1B = _BV(WGM12) | _BV(CS11);            // 16MHz/8/(1+99) = 20kHz
    TCCR1C = 0x00;
    OCR1A = 99;
    ...
    while (1) {
        ...
        case 0xE0:
            if (div_ratio != 20)                // if KEY1, decrement by 1
                div_ratio--;
            Set_timer1(div_ratio);
            break;
        case 0xD0:
            if (div_ratio != 200)               // if KEY2, increment by 1
                div_ratio++;
            Set_timer1(div_ratio);
            break;
        case 0xB0:
            div_ratio -= 10;                    // if KEY3, decrement by 10
            if (div_ratio < 20)
                div_ratio = 20;
            Set_timer1(div_ratio);
            break;
        case 0x70:
            div_ratio += 10;                    // if KEY4, increment by 10
            if (div_ratio > 200)
                div_ratio = 200;
            Set_timer1(div_ratio);
            break;
